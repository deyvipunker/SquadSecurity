...
namespace SquadSecurity.Frontend.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<TActionReponse>> PostAsync<T, TActionReponse>(string url, T model)
        {
            var messageJson = JsonSerializer.Serialize(model);
            var messageContent = new StringContent(messageJson, Encoding.UTF8, "application/json");
            var responseHttp = await _httpClient.PostAsync(url, messageContent);
            ...
            if (responseHttp.IsSuccessStatusCode)
            {
                var response = await UnserializeAnswerAsync<TActionReponse>(responseHttp);
                return new HttpResponseWrapper<TActionReponse>(response, false, responseHttp);
            }

            return new HttpResponseWrapper<TActionReponse>(default, true, responseHttp);
        }
